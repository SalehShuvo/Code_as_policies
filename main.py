...
# Capture and display environment image
image = env.get_camera_image()
image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

# Display image (Use appropriate method based on execution environment)
cv2.imshow("Environment View", image_rgb)  # Only works if running in a GUI-supported environment
cv2.waitKey(0)  # Wait for key press to close window
cv2.destroyAllWindows()

# Alternative: Save image if running in a non-GUI environment
cv2.imwrite("env_view.png", image_rgb)  # Saves the image to a file
...
# user_input = 'Put the pink block to the left of the rightmost bowl' #@param {allow-input: true, type:"string"}
...
# user_input = input("Enter a command for the robot (e.g., 'Put the pink block to the left of the rightmost bowl'): ")
user_input = 'Put the pink block to the left of the rightmost bowl'

# # Setup environment
# high_frame_rate = False  # Set based on your requirement
# env = PickPlaceEnv(render=True, high_res=False, high_frame_rate=high_frame_rate)
...
# Render video if frames are recorded
if env.cache_video:
    fps = 35 if high_frame_rate else 25
    rendered_clip = ImageSequenceClip(env.cache_video, fps=fps)

    # Show video (if GUI is available)
    try:
        rendered_clip.preview()  # Opens video in default media player
    except Exception:
        print("Could not preview video, saving instead...")

    # Save video to file
    rendered_clip.write_videofile("output_video.mp4", fps=fps)
    print("Video saved as output_video.mp4")
else:
    print("No video frames recorded.")
